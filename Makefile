...
GAIA_ALLAPPDIRS=$(shell find -L $(GAIA_DIR)$(SEP)apps $(GAIA_DIR)$(SEP)dev_apps -maxdepth 1 -mindepth 1 -type d  | sed 's@[/\\]@$(SEP_FOR_SED)@g')
...
b2g: node_modules/.bin/mozilla-download
	DEBUG=* ./node_modules/.bin/mozilla-download  \
		...
		--product b2g \
		--channel prerelease \
		--branch nightly/latest-mozilla-b2g34_v2_1 $@
...
ifneq ($(strip $(APP)),)
APP_TEST_LIST=$(shell find -L apps/$(APP) dev_apps/$(APP) -name '*_test.js' 2> /dev/null | grep '/test/unit/')
endif
